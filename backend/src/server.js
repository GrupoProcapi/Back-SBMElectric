...
app.put('/api/measurements-status/:id', validateUpdateMeasurements, async (req, res, next) => {
  const errors = validationResult(req);
  if(!errors.isEmpty())
    {
      return res.status(400).json({ errors: errors.array() });
...
app.put('/api/measurements-service/:id', validateUpdateMeasurements, async (req, res, next) => {
  const errors = validationResult(req);
  if(!errors.isEmpty())
    {
      return res.status(400).json({ errors: errors.array() });
  ...
  try {
    ...
    .then((row) => row ? 
        database.raw(`UPDATE measurements SET 
        sbmqb_service=${measurement.sbmqb_service ? "'"+measurement.sbmqb_service+"'" : 'sbmqb_service'}
        WHERE id = ${measurementId}`)
        .then(([rows]) => rows[0])
        .then((row) => res.json({ message: 'Measurement updated.' }))
    : res.status(404).json({ message: 'Measurement not found' }))
